...
<div class="city_modal_top">
    ...
    <input type="text"
           class="typical_input_bordered selector-city-search"
           data-id="<?= $area->id?>"
           placeholder="<?= \Yii::t('app', 'Введите город')?>"
    />
</div>
    ...
    <ul>
        <?php foreach ($cities as $item):?>
            <?php if (!empty($item['sort'])):?>
                <li><a href="javascript:void(0)" data-id="<?= $item['id']?>" data-country="<?= $area->country->translation->name?>" class="select-city"><?= $item['name']?></a></li>
            <?php else:?>
